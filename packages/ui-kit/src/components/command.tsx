...
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from './common/floating/dialog';
...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent className={cn('ui:overflow-hidden ui:p-0', className)} showCloseButton={showCloseButton}>
        <Command className="ui:**:data-[slot=command-input-wrapper]:h-12 ui:[&_[cmdk-group]:not([hidden])_~[cmdk-group]]:pt-0 ui:[&_[cmdk-input-wrapper]_svg]:h-5 ui:[&_[cmdk-input-wrapper]_svg]:w-5 ui:[&_[cmdk-item]_svg]:h-5 ui:[&_[cmdk-item]_svg]:w-5 ui:**:[[cmdk-group-heading]]:px-2 ui:**:[[cmdk-group-heading]]:font-medium ui:**:[[cmdk-group-heading]]:text-muted-foreground ui:**:[[cmdk-group]]:px-2 ui:**:[[cmdk-input]]:h-12 ui:**:[[cmdk-item]]:px-2 ui:**:[[cmdk-item]]:py-3">
          {children}
        </Command>
      </DialogContent>
    </Dialog>
...
function CommandInput({ className, ...props }: React.ComponentProps<typeof CommandPrimitive.Input>) {
  return (
    <div data-slot="command-input-wrapper" className="ui:flex ui:h-9 ui:items-center ui:gap-2 ui:border-b ui:px-3">
...
function CommandList({ className, ...props }: React.ComponentProps<typeof CommandPrimitive.List>) {
  return (
    <CommandPrimitive.List
      ...
      className={cn('ui:max-h-[300px] ui:scroll-py-1 ui:overflow-x-hidden ui:overflow-y-auto', className)}
      {...props}
    />
  );
}
...
function CommandEmpty({ ...props }: React.ComponentProps<typeof CommandPrimitive.Empty>) {
  return <CommandPrimitive.Empty data-slot="command-empty" className="ui:py-6 ui:text-center ui:text-sm" {...props} />;
}
