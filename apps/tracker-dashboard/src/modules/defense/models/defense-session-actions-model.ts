...
export const deleteDefenseSessionAction = action(async (sessionId: string) => {
  ...
  if (!response.ok) {
    throw new Error(
      response.error?.message ?? t('defense.session.toast.deleteError'),
    );
  }
...
export const rescheduleDefenseSessionAction = action(
  async (values: RescheduleDefenseSessionValues) => {
    ...
    if (!response.ok) {
      throw new Error(
        response.error?.message ?? t('defense.session.toast.rescheduleError'),
      );
    }
...
export const updateDefenseSessionAction = action(
  async (values: UpdateDefenseSessionValues) => {
    const response = await wrap(
    ...
    if (!response.ok) {
      throw new Error(
        response.error?.message ?? t('defense.session.toast.updateError'),
      );
    }
...
export const unregisterFromDefenseSessionAction = action(
  async (sessionId: string) => {
    const response = await wrap(unregisterFromDefenseSession(sessionId));
    if (!response.ok) {
      throw new Error(
        response.error?.message ?? t('defense.session.toast.unregisterError'),
      );
    }
...
export const defenseSessionDetailsQuery = query(
  async (sessionId: string) => {
    ...
    if (!response.ok) {
      throw new Error(
        response.error?.message ?? 'Failed to fetch defense session details',
      );
    }
    return response.data;
  },
...
export const isStudentRegisteredForSession = (
  session: DefenseSessionDetailsDTO | null,
  studentId: string,
): boolean => {
...
export const pendingDragRescheduleAtom = atom<PendingDragReschedule | null>(
  null,
