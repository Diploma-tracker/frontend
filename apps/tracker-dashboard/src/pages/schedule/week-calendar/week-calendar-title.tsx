...
export function WeekCalendarTitle() {
  ...
  const { title, weekRange, pickerOpen, setPickerOpen, setWeekRange, handleDayClick } = useWeekCalendar();

  return (
    <Popover open={pickerOpen} onOpenChange={setPickerOpen}>
      <PopoverTrigger asChild>
        <button
          className="-mx-1 cursor-pointer rounded px-1 text-left transition-colors hover:bg-accent hover:text-accent-foreground"
