...
export const COLOR_VAR_MAP: Record<
  string,
...
export function ScheduleEventContent({ eventInfo }: Props) {
  ...
  const colorKey =
    (event.classNames as string[]).find((c) => c in COLOR_VAR_MAP) ?? '';
  ...
  if (isCompact) {
    return (
      <div
        className={`fc-event-custom fc-event-custom--compact ${colors.bg} ${colors.text}`}
      >
        <span className={`fc-event-custom__bar ${colors.bar}`} />
        <span className="fc-event-custom__title truncate font-medium">
          {event.title}
        </span>
        <span className="fc-event-custom__time ml-auto shrink-0 opacity-70">
          {start}
        </span>
      </div>
    );
  ...
  return (
    <div className={`fc-event-custom ${colors.bg} ${colors.text}`}>
      ...
      <div className="fc-event-custom__body">
        <span className="fc-event-custom__title leading-tight font-semibold">
          {event.title}
        </span>
